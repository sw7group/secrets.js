...
function getRNG(type) {
  ...
  // react-native : react-native-aes-crypto.Aes.randomKey()
  async function reactNativeCryptoRandomKey(bits) {
    var bytes,
      radix,
      size,
      str = null;
    ...
    while (str === null) {
      str = construct(bits, await crypto.randomKey(bytes), radix, size);
    }
  ...
  if (type && type === 'testRandom') {
    ...
    return testRandom;
  } else if (type && type === 'reactNativeCryptoRandomKey') {
    config.typeCSPRNG = type;
    return reactNativeCryptoRandomKey;
  } else {
    return reactNativeCryptoRandomKey;
  }
}
...
async function getShares(secret, numShares, threshold) {
  var shares = [],
    coeffs = [secret],
    i,
    len;
  ...
  for (i = 1; i < threshold; i++) {
    coeffs[i] = parseInt(await config.rng(config.bits), 2);
  }
...
export function init(bits, rngType) {
  var logs = [],
    exps = [],
    x = 1,
    primitive,
...
export function combine(shares, at) {
  let i,
    j,
    len,
    len2,
...
export function extractShareComponents(share) {
  var bits,
    id,
    idLen,
    max,
...
export function str2hex(str, bytesPerChar) {
  var hexChars,
    max,
    out = '',
    neededBytes,
...
export function hex2str(str, bytesPerChar) {
  var hexChars,
    out = '',
    i,
    len;
...
export async function split(secret, numShares, threshold, padLength) {
  var neededBits,
    subShares,
  ...
  if (typeof threshold !== 'number' || threshold % 1 !== 0 || threshold < 1) {
    throw new Error(
      'Threshold number of shares must be an integer between 1 and 2^bits-1 (' +
        config.maxShares +
        '), inclusive.',
    );
  }
  ...
  for (i = 0, len = secret.length; i < len; i++) {
    subShares = await getShares(secret[i], numShares, threshold);
...
export function newShare(id, shares) {
  ...
  if (id && radid && shares && shares[0]) {
    ...
    return constructPublicShareString(share.bits, radid, combine(shares, id));
  }
...
export const _padLeft = padLeft;
export const _hex2bin = hex2bin;
export const _bin2hex = bin2hex;
export const _getRNG = getRNG;
export const _isSetRNG = isSetRNG;
export const _splitNumStringToIntArray = splitNumStringToIntArray;
export const _horner = horner;
export const _lagrange = lagrange;
export const _getShares = getShares;
export const _constructPublicShareString = constructPublicShareString;
/* end-test-code */
